...
int main(int argc, char * argv[]) 
{
    ...
    char* theConfigFilePath = "./easynvr.cfg";
    char* theXMLFilePath = "./easynvr.xml";
   ...
   qtss_printf("%s/%s ( Build/%s; Platform/%s; %s%s) Built on: %s\n",QTSServerInterface::GetServerName().Ptr,
    ...
    while ((ch = getopt(argc,argv, "vdxp:o:c:irsS:I")) != EOF) // opt: means requires option
    {
        switch(ch)
        {
            case 'v':
                ...
                qtss_printf("-i: Install the EasyNVR service\n");
                qtss_printf("-r: Remove the EasyNVR service\n");
                qtss_printf("-s: Start the EasyNVR service\n");
            ...
            case 'x':
                ...
                break;
            case 'i':
                qtss_printf("Installing the EasyNVR service...\n");
                ::InstallService("EasyNVR");
                qtss_printf("Starting the EasyNVR service...\n");
                ::RunAsService("EasyNVR");
                ::exit(0);
                break;
            case 'r':
                qtss_printf("Removing the EasyNVR service...\n");
                ::RemoveService("EasyNVR");
                ::exit(0);
            case 's':
                qtss_printf("Starting the EasyNVR service...\n");
                ::RunAsService("EasyNVR");
    ...
    {
        qtss_printf("EasyNVR has expired\n");
        ::exit(0);
    }
    ...
    qtss_printf("EasyNVR must either be started from the DOS Console\n");
    qtss_printf("using the -d command-line option, or using the Service Control Manager\n\n");
    qtss_printf("Waiting for the Service Control Manager to start EasyNVR...\n");
...
void InstallService(char* inServiceName)
{
    ...
    if (!theSCManager)
    {
        qtss_printf("Failed to install EasyNVR Service\n");
        return;
    }
    ...
    {
        ::CloseServiceHandle(theService);
        qtss_printf("Installed EasyNVR Service\n");
    }
    else
        qtss_printf("Failed to install EasyNVR Service\n");
...
void RemoveService(char *inServiceName)
{
    ...
    if (!theSCManager)
    {
        qtss_printf("Failed to remove EasyNVR Service\n");
        return;
    }
    ...
    if (theService != NULL)
    {
        ...
        ::CloseServiceHandle(theService);
        qtss_printf("Removed EasyNVR Service\n");
    }
    else
        qtss_printf("Failed to remove EasyNVR Service\n");
